...
Imperative access control rules are functions which return either a boolean value or a [declarative](#declarative-list) value (e.g. a GraphQL `where` clause).
...
If the function returns a declarative value then this will be applied to the operation as decribed [above](#declarative-list).
...
| `operation`     | The CRUD operation being performed (`'create'`, `'read'`, `'update'`).                                                                                        |
| `session`       | The current session object. See the [Sessions API](./session) for details                                                                                     |
...
| `context`       | The [`KeystoneContext`](./context) object of the originating GraphQL operation.                                                                               |
| `item`          | The item being updated, deleted, or read. This object is an unresolved list item. See the [list item API](./items) for more details on unresolved list items. |
